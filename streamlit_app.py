...
SYM2ENSG: Dict[str, str] = {
    ...
    "GAB1": "ENSG00000117676",
    "ALK": "ENSG00000171094",
    "EML4": "ENSG00000143924",
...
def download_tpm() -> Path:
    ...
    with st.spinner("Downloading Toil TPM expression matrix (~1.7 GB)…"):
...
def selection_statistics(selections: Sequence[Sequence[str]], drugs: Sequence[str]) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    freq_df = pd.DataFrame(
        {
            "drug": drugs,
            "count": [freq.get(d, 0) for d in drugs],
        }
    )
...
def synthetic_classification(selections: Sequence[Sequence[str]], P: pd.DataFrame) -> Tuple[pd.DataFrame, float, float]:
    panel = example_drug_panel()
    drugs = list(panel.keys())
    N = len(selections)
...
def plot_heatmap(P: pd.DataFrame) -> plt.Figure:
    ...
    Ps = P.loc[keep]
    Ps_vis = (Ps - Ps.mean(axis=1).values.reshape(-1, 1)) / (
        Ps.std(axis=1).values.reshape(-1, 1) + 1e-8
    )
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        base_overlap = st.slider("Penalty per shared pathway", min_value=0.0, max_value=1.0, value=0.25, step=0.05)
